...
InputPOGamma( void )
{
  ...
  // 戦術的特徴の読み込み
  {
    string path = po_parameters_path + "nTacticalFeature1.txt";
    cerr << po_parameters_path << endl;
    cerr << "read " << path << endl;
    ifstream in {path};
    for (int i = 0; i < PO_TACTICALS_MAX1; i++) {
      in >> po_tactical_set1[i];
    }
  }

  {
    string path = po_parameters_path + "nTacticalFeature2.txt";
    cerr << "read " << path << endl;
    ifstream in {path};
    for (int i = 0; i < PO_TACTICALS_MAX2; i++) {
      in >> po_tactical_set2[i];
    }
  }
  ...
  // 直前の着手からの距離の読み込み
  {
    string path = po_parameters_path + "nPreviousDistance.txt";
    cerr << "read " << path << endl;
    ifstream in {path};
    for (int i = 0; i < PREVIOUS_DISTANCE_MAX + 1; i++) {
      double w;
      in >> w;
      po_previous_distance[i] = w;
    }
    for (int i = 0; i < PREVIOUS_DISTANCE_MAX; i++) {
      po_previous_distance[i] = exp(po_previous_distance[i] - po_previous_distance[PREVIOUS_DISTANCE_MAX]);
    }
  }
  ...
  for (int i = 0; i < PREVIOUS_DISTANCE_MAX - 1; i++) {
    ...
    cerr << po_neighbor_orig[i] << " -> " << po_previous_distance[i] << endl;
  }
  ...
  */
  ...
  {
    // 直前の着手からの距離の読み込み
    string path = po_parameters_path + "PreviousDistance.txt";
  ...
  // MD2のパターンの読み込み
  {
    fill_n(po_pattern, MD2_MAX, 0.0f);

    string path = po_parameters_path + "nMD2.txt";
    cerr << "read " << path << endl;
    ifstream in{ path };
    string line;
